...
void ABuildingExplosionTrigger::OnPlayerOverlap( UPrimitiveComponent* OverlappedComponent , AActor* OtherActor , UPrimitiveComponent* OtherComp , int32 OtherBodyIndex , bool bFromSweep , const FHitResult& SweepResult )
{
	...
	GetWorldTimerManager().SetTimer( Handle1 , FTimerDelegate::CreateLambda(
		[this]() { 
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_1, 1.0f, 1.0f );
		}), 2.0f, false);
	...
	GetWorldTimerManager().SetTimer( Handle2 , FTimerDelegate::CreateLambda(
		[this]() { 
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_2 , 1.0f , 1.0f );
		} ) , 2.5f , false );
	...
	GetWorldTimerManager().SetTimer( Handle3 , FTimerDelegate::CreateLambda(
		[this]() { 
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_1 , 1.7f , 1.0f );
		} ) , 3.1f , false );

	FTimerHandle ShakeHandle;
	GetWorldTimerManager().SetTimer( ShakeHandle , FTimerDelegate::CreateLambda(
		[this]() {
			UGameplayStatics::PlayWorldCameraShake( GetWorld() , BuildingShakeClass , FVector( -349.3f , 15388.96f , 6000 ), InnerRadius , OuterRadius );
		} ) , 4.0f , false );
	...
	GetWorldTimerManager().SetTimer( Handle4 , FTimerDelegate::CreateLambda(
		[this]() {
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_1 , 1.0f , 0.8f );
		} ) , 4.2f , false );
	...
	GetWorldTimerManager().SetTimer( Handle5 , FTimerDelegate::CreateLambda(
		[this]() { 
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_2 , 1.0f , 1.2f );
		} ) , 5.2f , false );
	...
	GetWorldTimerManager().SetTimer( Handle6 , FTimerDelegate::CreateLambda(
		[this]() {
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_1 , 1.0f , 1.2f );
		} ) , 6.2f , false );
	...
	GetWorldTimerManager().SetTimer( Handle7 , FTimerDelegate::CreateLambda(
		[this]() {
			...
			UGameplayStatics::PlaySound2D( GetWorld() , SFX_Explosion_2 , 1.0f , 0.8f );
		} ) , 7.2f , false );
